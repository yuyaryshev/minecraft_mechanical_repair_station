...
public class mechanicalrepairstationMenu extends RecipeBookMenu<CraftingContainer> {
    ...
    private final mechanicalrepairstationBlockEntity workbench;
    ...
    public mechanicalrepairstationMenu(int id, Inventory inventory, ContainerLevelAccess access,
                                   mechanicalrepairstationBlockEntity workbench) {
        super(MWMenus.mechanical_repair_station.get(), id);
        this.access = access;
    ...
    public static mechanicalrepairstationMenu fromNetwork(int id, Inventory inventory, FriendlyByteBuf buf) {
        ...
        mechanicalrepairstationBlockEntity workbench = be instanceof mechanicalrepairstationBlockEntity
                ? (mechanicalrepairstationBlockEntity) be
                : null;
        return new mechanicalrepairstationMenu(id, inventory, ContainerLevelAccess.create(inventory.player.level(), pos), workbench);
    }
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        if (!mechanicalrepairstationBlockEntity.isManualChargeEnabled())
            return false;
        if (player.level().isClientSide || workbench == null)
            return false;
        if (workbench.getRotationBuffer() >= mechanicalrepairstationBlockEntity.maxRotations())
            return false;
        ...
        int amount = mechanicalrepairstationBlockEntity.manualChargeAmount();
        if (amount <= 0)
            return false;
    ...
    private static class PoweredResultSlot extends ResultSlot {
        ...
        public PoweredResultSlot(Player player, CraftingContainer craftSlots, ResultContainer resultSlots, int index,
                                 int x, int y, mechanicalrepairstationBlockEntity workbench) {
